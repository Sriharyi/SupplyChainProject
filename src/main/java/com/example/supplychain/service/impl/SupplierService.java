...
import com.example.supplychain.exception.EntityNotNullException;
import com.example.supplychain.exception.NoSupplierFoundException;
...
public class SupplierService implements SupplierServiceInterface {
   ...
   public Supplier getById(String id) {
      ...
      if (result.isEmpty()) {
         throw new NoSupplierFoundException(id);
      }
      return result.get();
   }
   ...
   public Supplier saveData(Supplier supplier) {
      ...
      if (supplier != null) {
         ...
         throw new EntityNotNullException();
      }
      return supp;
   }
   ...
   public Boolean updateImage(String id, MultipartFile image) {
      Supplier entity = repo.findById(id)
            .orElseThrow(() -> new NoSupplierFoundException());
      Boolean result = false;
      try {
         if (isImage(image)) {
   ...
   public Boolean deleteImage(String id) {
      ...
      try {
         ...
         result = file.delete();
         // result = Files.deleteIfExists(new
         // File(supplier.getSupplierlogoPath()).toPath());
      } catch (Exception e) {
         e.printStackTrace();
      }
